...
export default function AdminRoadmapDetailPage() {
  ...
  useEffect(() => {
    ...
    const parsedId = Number(searchParams.get("id") ?? 0);
    setRoadmapId(Number.isFinite(parsedId) ? parsedId : 0);
  }, []);
  ...
  async function refresh() {
    if (!roadmapId) {
      setRoadmap(null);
      setError("Roadmap tidak ditemukan atau parameter id belum tersedia.");
      return;
    }
  ...
  async function saveRoadmap() {
    ...
    try {
      ...
      if (roadmapModal?.mode === "create") router.push(`/admin/roadmap/detail?id=${saved.id_roadmap}`);
      else await refresh();
    } catch (err) {
