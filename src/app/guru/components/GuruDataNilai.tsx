...
import { getGuruWorkspace, type GuruWorkspace } from "../../../features/guru/api";
...
function isSmaSchool(jenisSekolah?: string | null) {
  return String(jenisSekolah || "SMA").toUpperCase() === "SMA";
}

function isSemesterUmumSma(jenisSekolah: string | null | undefined, semester: string) {
  return isSmaSchool(jenisSekolah) && ["1", "2"].includes(semester);
}

function shouldShowJurusan(jenisSekolah: string | null | undefined, selectedSemester: string) {
  if (!isSmaSchool(jenisSekolah)) return true;
  return !isSemesterUmumSma(jenisSekolah, selectedSemester);
}

function getStudentId(siswa: NilaiMatrixStudent) {
  return String(siswa.id_siswa ?? siswa.id ?? siswa.studentId ?? "");
}

function getStudentName(siswa: NilaiMatrixStudent) {
  return String(siswa.nama ?? siswa.studentName ?? siswa.nisn ?? "-");
}

function getDisplayKelas(jenisSekolah: string | null | undefined, semester: string, siswa: NilaiMatrixStudent) {
  if (!isSmaSchool(jenisSekolah)) {
    return siswa.kelas || siswa.className || "-";
  }
...
export function GuruDataNilai() {
  ...
  const jenisSekolah = workspace?.sekolah?.jenis ?? "SMA";
  ...
  const selectedJurusanName = useMemo(() => {
    ...
    return String(found?.nama ?? selectedJurusan);
  ...
  async function loadWorkspace() {
    ...
    try {
      const data = await getGuruWorkspace();
      setWorkspace(data);
    } catch (err) {
      setErrorMessage(err instanceof Error ? err.message : "Gagal memuat ruang kerja guru.");
    } finally {
  ...
  async function loadNilaiMatrix(page = currentPage) {
    ...
    try {
      const result = await apiFetch<NilaiMatrixResponse>(`/guru/nilai-matrix?${params.toString()}`, {
        method: "GET",
        alert: false,
      });
      ...
      setMapelColumns([]);
      setTotalStudents(0);
      setErrorMessage(err instanceof Error ? err.message : "Data nilai belum bisa dimuat.");
    } finally {
      setHasLoadedMatrixOnce(true);
  ...
  useEffect(() => {
    ...
    if (selectedJurusan === "semua" && jurusanRows.length > 0) {
      setSelectedJurusan(String(jurusanRows[0].id));
    }
  ...
  }, [loadingWorkspace, workspace, currentPage, selectedSemester, selectedJurusan, selectedKelas, debouncedSearch, jurusanFilterAktif]);

  const totalPages = Math.max(1, Math.ceil(totalStudents / ITEMS_PER_PAGE));
  const isWaitingSearch = searchTerm !== debouncedSearch;
  const isLoading = loadingWorkspace || loadingMatrix || isWaitingSearch;
  ...
  function getJurusanLabel(siswa: NilaiMatrixStudent) {
  ...
  return (
    <section id="nilai" className="scroll-mt-28 rounded-[2rem] border border-sky-100 bg-white shadow-sm shadow-sky-100/60">
        ...
        <div className="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-3xl">
            ...
            <p className="mt-2 max-w-2xl text-sm font-medium leading-6 text-slate-600">
              Data nilai dimuat per halaman dari server, jadi Guru BK tidak perlu menunggu semua siswa dan semua nilai dimuat sekaligus.
            </p>
          </div>
      ...
      <div className="space-y-6 p-5 md:p-6">
        ...
        <section className="overflow-hidden rounded-3xl border border-sky-100 bg-white shadow-sm shadow-sky-100/60">
          <div className="flex flex-col gap-4 border-b border-sky-100 bg-gradient-to-r from-sky-50 via-white to-blue-50 px-5 py-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-1 max-w-2xl text-sm font-medium leading-6 text-slate-500">
                Pencarian diberi jeda singkat agar server tidak ditembak request setiap satu huruf diketik.
              </p>
            </div>
          ...
          <div className="grid gap-3 p-5 md:grid-cols-3 xl:grid-cols-[1.2fr_0.75fr_0.9fr_0.75fr_auto]">
            ...
            >
              <option value="semua">
                {jurusanFilterAktif ? "Pilih jurusan" : "Semester 1 dan 2 SMA tidak memakai jurusan"}
              </option>
              {jurusanFilterAktif &&
            ...
            <button
              ...
              disabled={isLoading || !workspace?.canAccessWorkspace}
              className="inline-flex items-center justify-center gap-2 rounded-2xl border border-sky-100 bg-sky-50 px-4 py-2.5 text-sm font-bold text-sky-700 transition hover:-translate-y-0.5 hover:border-sky-200 hover:bg-sky-100 hover:shadow-sm disabled:cursor-not-allowed disabled:opacity-50"
            >
              <Icon name="refresh" className="h-4 w-4" />
            ...
            <div className={`overflow-x-auto ${showTableOverlay ? "pointer-events-none select-none opacity-70" : ""}`}>
              <table className="min-w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-100 bg-white">
                  {matrixRows.length === 0 ? (
                    <tr>
                      <td colSpan={4 + mapelColumns.length} className="px-5 py-14 text-center">
                    ...
                    matrixRows.map((siswa) => {
                      const siswaId = getStudentId(siswa);
                      ...
                      return (
                        <tr key={siswaId} className="transition hover:bg-sky-50/50">
                          <td className="sticky left-0 z-10 min-w-[220px] bg-white px-5 py-4 font-semibold text-slate-900">
                            {getStudentName(siswa)}
                          </td>
