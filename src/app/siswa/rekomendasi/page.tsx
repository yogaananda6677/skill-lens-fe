...
function RoadmapGenerationOverlay({
  open,
  autoMode,
}: {
  open: boolean;
        ...
        <div className="relative">
          ...
          <div className="mt-6 grid gap-3">
            {stages.map((stage, index) => (
              <div
                key={stage}
                className="flex items-center gap-3 rounded-2xl bg-sky-50/70 p-3 text-left text-sm font-bold text-slate-600 ring-1 ring-sky-100 skilllens-smooth"
              >
...
export default function SiswaRekomendasiPage() {
  ...
  async function handleProcessSpk(options?: { autoGenerateRoadmap?: boolean }) {
    ...
    try {
      ...
      if (options?.autoGenerateRoadmap) {
        ...
        await generateRoadmapFromRecommendation(topRecommendation, {
          auto: true,
        });
      ...
      showSuccess(
        "Rekomendasi berhasil diproses",
        "Pilih salah satu kartu hasil, lalu generate roadmap untuk mulai belajar.",
      );
  ...
  useEffect(() => {
    ...
    if (loadingProfile || loadingLatest || processing || generatingRoadmap) {
      return;
    }
