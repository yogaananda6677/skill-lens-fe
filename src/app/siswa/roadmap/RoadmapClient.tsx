...
import { getSiswaRoadmap } from "../../../features/siswa/api";
import type { CareerRoadmap, RoadmapStep } from "../../../features/siswa/types";
...
function StepCard({
  ...
  return (
    <article
      className={`rounded-[1.5rem] border bg-white p-5 shadow-lg shadow-slate-900/5 transition ${
        active ? "border-[#0f2a5f] ring-4 ring-indigo-50" : "border-slate-100"
      }`}
    >
      <div className="flex items-start gap-4">
        <button
          type="button"
          onClick={onToggle}
          className={`grid h-12 w-12 shrink-0 place-items-center rounded-2xl text-sm font-semibold transition ${
            done ? "bg-emerald-500 text-white" : "bg-slate-100 text-slate-500 hover:bg-cyan-200 hover:text-slate-950"
          }`}
          aria-label={`Tandai ${step.title}`}
        >
          {done ? "✓" : index + 1}
        </button>
        <button type="button" onClick={onOpen} className="min-w-0 flex-1 text-left">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#0f2a5f]">
...
function emptyRoadmap(): CareerRoadmap {
  return {
    careerId: "default",
...
export default function RoadmapClient() {
  ...
  const initialCareerId = searchParams.get("career") || "default";

  const [careerId, setCareerId] = useState(initialCareerId);
  const [roadmap, setRoadmap] = useState<CareerRoadmap>(emptyRoadmap());
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        setError("");
        const data = await getSiswaRoadmap(careerId);

        if (cancelled) return;

        setRoadmap(data);
        const initialCompletedIds = data.steps
          .slice(0, data.initialCompleted)
          .map((step) => step.id);

        setCompletedStepIds(initialCompletedIds);
  ...
  const activeStep = useMemo(() => {
    return roadmap.steps.find((step) => step.id === activeStepId) ?? roadmap.steps[0];
  }, [activeStepId, roadmap.steps]);

  const progress = roadmap.steps.length
    ? Math.round((completedStepIds.length / roadmap.steps.length) * 100)
    : 0;

  const totalDuration = roadmap.steps
    .map((step) => step.duration)
    .filter((duration) => !duration.toLowerCase().includes("berkelanjutan"))
    .join(" + ");
  ...
  return (
    <DashboardShell
      ...
      subtitle="Ikuti langkah belajar berdasarkan arah karier yang dipilih."
      userName="Siswa"
      userLabel="Siswa"
      ...
      rightSlot={
        <Link
          href="/siswa#hasil"
          className="hidden rounded-2xl border border-slate-200 bg-white px-4 py-2 text-xs font-medium text-slate-700 shadow-sm transition hover:border-blue-200 hover:text-blue-700 md:inline-flex"
        >
          Ganti hasil
        </Link>
      }
    >
      {error && (
        <div className="mb-5 rounded-2xl bg-rose-50 p-4 text-sm font-medium text-rose-700 ring-1 ring-rose-100">
          {error}
        </div>
      )}

      {loadingRoadmap ? (
        <div className="rounded-3xl bg-white p-6 text-sm font-medium text-slate-500 shadow-sm">Memuat roadmap...</div>
      ) : (
        <>
              ...
              <article key={label} className="rounded-[1.4rem] border border-slate-100 bg-white p-5 shadow-lg shadow-slate-900/5">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="grid h-11 w-11 place-items-center rounded-2xl bg-blue-50 text-blue-700 ring-1 ring-blue-100">
                    <Icon name={icon as "check" | "roadmap" | "target" | "clock"} className="h-5 w-5" />
                  </div>
                </div>
            ...
            <div className="space-y-6">
              <Panel className="bg-slate-950 text-white">
                ...
                <p className="mt-2 text-xs font-semibold text-slate-300">
                  {completedStepIds.length} dari {roadmap.steps.length} tahap selesai
                </p>
              </Panel>

              {/* Untuk sekarang, “Ganti arah karier” diarahkan lewat link hasil SPK (agar careerId datang dari query).
                  Nanti bisa ditambah UI list roadmap bila backend support multiple templates. */}
              <Panel>
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#0f2a5f]">Arah karier</p>
                <p className="mt-3 text-sm font-semibold leading-6 text-slate-600">
                  Gunakan halaman <Link href="/siswa" className="font-bold text-slate-950">Hasil SPK</Link> untuk mengganti arah karier.
                </p>
              </Panel>
            </div>
                ...
                <Panel>
                  ...
                  <div className="mt-5">
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {activeStep.checklist.map((item) => (
                        <span key={item} className="rounded-full bg-[#eef5ff] px-3 py-1 text-xs font-semibold text-[#4c51d9]">
                          {item}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                    <button
                      ...
                      className="rounded-full bg-slate-950 px-6 py-4 text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:bg-[#0f2a5f]"
                    >
                      {completedStepIds.includes(activeStep.id) ? "Batalkan selesai" : "Tandai selesai"}
                    </button>
                    <Link
                      href="/siswa"
                      className="rounded-full border border-slate-200 bg-white px-6 py-4 text-center text-sm font-semibold text-slate-700 transition hover:-translate-y-0.5 hover:border-[#0f2a5f]/30 hover:text-[#0f2a5f]"
                    >
                      Kembali ke profil
                    </Link>
                  </div>
